...
sound_email_off=~/.config/polybar/scripts/sound_email_off
notf_on=~/.config/polybar/scripts/notf_on

if [ $email_count -eq 0 ]; then
    echo "📭"
    if [ -f $sound_email_off ]; then
        rm -f $sound_email_off
    fi   

elif [ $email_count -gt 0 ] && [ ! -f "$sound_email_off" ] && [ -f $notf_on ] && ! pgrep -x "vlc|elisa|kodi" > /dev/null; then
    echo "📬 $email_count"
...
elif [ -f $notf_on ] && [ $email_count -gt 0 ]; then
    echo "📬 $email_count"
    kdialog --icon "$HOME/.local/share/icons/GoVi-Ic/apps/64/gmail.svg" --passivepopup "$email_title" 8 &
