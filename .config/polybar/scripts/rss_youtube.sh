...
CHECK_ENTRIES(){
    ...
    echo -e "\nREAL SPIRIT DYNAMICS:\n$(curl -s "https://www.youtube.com/feeds/videos.xml?channel_id=UCRwExXfoGXMCwkArli449Ag" | awk -F "<media:title>|</media:title>" 'NF>1 {print $2}')"
    ...
    echo -e "\nZohar StarGate:\n$(curl -s "https://www.youtube.com/feeds/videos.xml?channel_id=UC8UexTu0pRdzJU83eLXGPIA" | awk -F "<media:title>|</media:title>" 'NF>1 {print $2}')"
    # END
...
UPDATES="$(diff -s "$LOGS"/check_entries_youtube "$LOGS"/recheck_entries_youtube)"
UPDATES_COUNT="$($UPDATES | grep -c '^>')"

echo "$UPDATES_COUNT" > "$LOGS"/rss_youtube
...
# crontab -e
# */5 * * * * ~/.config/polybar/scripts/rss_youtube.sh
# crontab -l